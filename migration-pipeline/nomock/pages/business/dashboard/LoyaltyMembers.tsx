...
// Original declaration removed: const mockLoyaltyMembers = [...{
  id: 'u1',
  name: 'Sarah Johnson',
...
// Mock data for program metrics
// TODO: Replace mock data with Supabase query in loader
// Original declaration removed: const mockProgramMetrics = {...
  totalMembers: 458,
  activeMembers: 312,
  newMembersThisMonth: 28,
  averagePointsPerMember: 580,
  totalPointsIssued: 265400,
  totalPointsRedeemed: 98500,
  redemptionRate: 37.1,
  topTiers: [{
    name: 'Bronze',
    count: 245,
...
export function LoyaltyMembers() {
  ...
  return <div className="container mx-auto px-4 py-6">
      ...
      {/* Notification Modal */}
      {showNotificationModal && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            ...
            <div className="p-6">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-xl font-bold text-gray-900">
                    Send{' '}
                    {notificationType === 'email' ? 'Email' : notificationType === 'sms' ? 'SMS' : 'Push Notification'}
                  </h3>
                  <p className="text-sm text-gray-600 mt-1">
                    To {selectedMembers.length} selected{' '}
              ...
              {showSuccessMessage ? <div className="text-center py-6">
                  ...
                  <p className="mt-2 text-sm text-gray-500">
                    Your{' '}
                    {notificationType === 'email' ? 'email' : notificationType === 'sms' ? 'SMS' : 'push notification'}{' '}
                    has been sent to {selectedMembers.length}{' '}
                    {selectedMembers.length === 1 ? 'member' : 'members'}.
                  </p>
                </div> : <div>
                  ...
                  <div className="flex justify-between">
                    ...
                    <button onClick={handleSendNotification} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 flex items-center" disabled={!notificationMessage || notificationType === 'email' && !notificationSubject}>
                      ...
                      Send{' '}
                      {notificationType === 'email' ? 'Email' : notificationType === 'sms' ? 'SMS' : 'Notification'}
                    </button>
                  </div>
                </div>}
            </div>
